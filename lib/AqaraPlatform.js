...
AqaraPlatform.prototype.parseMessage = function(msg, rinfo){
  ...
  if (cmd === 'iam') {
    ...
    for(var index in data) {
      ...
      // platform.log.debug("Set %s's password to %s", deviceSid, this.passwords[gatewaySid]);
...
LightSwitchCommander.prototype.send = function(on) {
  ...
  if (!password) {
    platform.log.error("No password for %s, please edit ~/.homebridge/config.json", this.deviceSid);
    return;
  }
